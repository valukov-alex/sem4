...
/*	for (int i = 2; i < map.n - 1; i = i + 1)		//  цикл по всем
		for (int j = 2; j < map.m - 1; j = j +1) {
			particle.position = map.baskets[i][j].coord;
			int rnd = rand() % 360;
			Vector2 t(sin(rnd), cos(rnd));
			particle.velocity = t * BALLS_SPEED;
			particle.previosPosition = particle.position - particle.velocity * dt1 ;
			if (i < map.n / 2)
				particle.clr = sf::Color::Blue;
			else
	...
	particle.clr = sf::Color::Red;
	map.push_prtcls_into_vessel(2, 0.0008, particle);
